...
class YoloTrainingNode(Node):
    def __init__(self):
        super().__init__('yolo_training_node')
    ...
    def run_training(self, msg):
        ...
        base_cfg = msg
        val_data_cfg = DataConfig(batch_size=8,image_size=[640, 640],cpu_num=16,shuffle=True,pin_memory=True,source='dev',data_augment={},dynamic_shape=False)
        ...
        self.data_cfg = DataConfig(batch_size=8,image_size=[640, 640],cpu_num=16,shuffle=True,pin_memory=True,data_augment={},dynamic_shape=False,source='dev')
        ...
        self.cfg = Config(name='v9-s-3', task=self.train_cfg, dataset=dataset, model=model, device='cuda', image_size=[640, 640], out_path='runs', exist_ok=True, lucky_number=10, use_wandb=False, use_tensorboard=False, weight=True, cpu_num=16)
        
        # if base_cfg.task_type == gs_ui_msgs.msg.BaseConfig.TASK_TYPE_TRAIN:
        #     self.train_cfg = base_cfg.train
        # elif base_cfg.task_type == gs_ui_msgs.msg.BaseConfig.TASK_TYPE_INFER:
        #     self.inference_cfg = base_cfg.inference
        # elif base_cfg.task_type == gs_ui_msgs.msg.BaseConfig.TASK_TYPE_VALID:
        #     self.validation_cfg = base_cfg.validation
            
        # self.cfg = Config(task=self.train_cfg, dataset=base_cfg.dataset, model=base_cfg.model, device=base_cfg.device, image_size=640, out_path=base_cfg.out_path, exist_ok=base_cfg.exist_ok, lucky_number=base_cfg.lucky_number, use_wandb=base_cfg.use_wandb, use_tensorboard=base_cfg.use_tensorboard, weight=base_cfg.weight)
        
        
        
        # self.train_cfg.task = 'train'
        # self.train_cfg.epochs = msg.epochs
